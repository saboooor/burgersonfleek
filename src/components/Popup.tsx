import { $, component$, useOnDocument, useSignal } from '@qwik.dev/core';
import IconInBag from './svg/IconInBag';
import Citrus from 'lucide-icons-qwik/icons/Citrus';
import { Link } from '@qwik.dev/router';

export default component$(
  ({
    fixed,
    class: Class,
    col,
    types,
    pathname,
  }: {
    fixed?: boolean;
    class?: { [key: string]: boolean };
    col?: boolean;
    types: ('deal' | 'order')[];
    pathname?: string;
  }) => {
    const prevScrollpos = useSignal(0);
    const PopupRefBottom = useSignal<HTMLDivElement>();
    const PopupRefTop = useSignal<HTMLDivElement>();

    useOnDocument(
      'scroll',
      $(() => {
        if (!fixed) return;

        if (
          prevScrollpos.value > window.scrollY &&
          window.scrollY + 1000 < document.body.scrollHeight
        ) {
          if (PopupRefBottom.value) PopupRefBottom.value.style.bottom = '0';
          if (PopupRefTop.value) PopupRefTop.value.style.top = '0';
        } else if (window.scrollY > 100) {
          if (PopupRefBottom.value)
            PopupRefBottom.value.style.bottom =
              PopupRefBottom.value.offsetHeight * -1 + 'px';
          if (PopupRefTop.value)
            PopupRefTop.value.style.top =
              PopupRefTop.value.offsetHeight * -2 + 'px';
        }
        prevScrollpos.value = window.scrollY;
      })
    );

    return (
      <>
        <div
          class={{
            'fixed bottom-0 flex w-full flex-col items-center justify-center p-4 pb-18 transition-all sm:pb-4':
              fixed,
            'hidden sm:flex': col,
            flex: !col,
            ...Class,
          }}
          ref={PopupRefBottom}
        >
          {types.includes('order') && (
            <div
              class={{
                'lum-card lum-grad-bg-burger-800/40 text-lum-text! max-w-7xl items-center justify-between gap-2 p-2': true,
                'flex-row': !col,
                'font-futura backdrop-blur-md': true,
              }}
            >
              <p
                class={{
                  'mx-2 sm:text-xl': true,
                  'my-1': col,
                }}
              >
                Feeling HANGRY?
              </p>
              <a
                target="_blank"
                data-umami-event="order_online"
                href="https://order.toasttab.com/online/burgers-on-fleek-135-harwood-ave-n-unit-b212"
                class={{
                  'lum-btn lum-btn-p-2 rounded-lum-2 text-sm sm:text-lg': true,
                  'w-full justify-center': col,
                  'hover:text-lum-text! border-none': true,
                  'lum-grad-bg-burger-600 from-burger-600 to-burger-700 hover:from-burger-500 active:from-burger-500 bg-linear-to-b': true,
                }}
              >
                <IconInBag class="w-5 fill-current sm:mr-1 sm:w-auto" /> Order
                Online
              </a>
            </div>
          )}
        </div>
        <div
          class={{
            'mt-16': !pathname?.includes('menu'),
            'mt-30 sm:mt-16': pathname?.includes('menu'),
            'fixed top-0 flex w-full flex-col items-center justify-center p-4 transition-all':
              fixed,
            'hidden sm:flex': col,
            flex: !col,
            ...Class,
          }}
          ref={PopupRefTop}
        >
          {types.includes('deal') && (
            <div
              class={{
                'lum-card lum-btn-p-2 text-lum-text! max-w-7xl items-center justify-between gap-2': true,
                'flex-row': !col,
                'font-futura backdrop-blur-md': true,
              }}
            >
              <Citrus size={20} />
              <p
                class={{
                  'mx-2 sm:text-xl': true,
                  'my-1': col,
                }}
              >
                <span class="font-bold">
                  Try out our new Yuzu Fizz!{' '}
                  <Link href="/menu#drinks" class="text-lum-accent underline">
                    Order now
                  </Link>
                </span>
              </p>
            </div>
          )}
        </div>
      </>
    );
  }
);
